...
| --- | --- | --- |
| `Inochi2D/inochi-creator` `v0_8` — `source/creator/viewport/common/automesh/{automesh,contours,grid}.d` | [source] | **the editor already has an automatic mesher**: `AutoMeshProcessor.autoMesh(Drawable, IncMesh, …) -> IncMesh` is a pure computation (GUI coupling is only in `configure()`/`icon()`); contour mode = alpha → bwlabel → findContours → centroid moment → resample → scale/erode-dilate, grid mode = opaque bounds → divideAxes → lattice. **No call site exists** (no CLI/dylib/IPC) — expose it through the bridge instead of porting it. |
